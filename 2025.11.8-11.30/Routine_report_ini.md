...
8. 后为严谨讨论，也讨论了 $ \mathbf{y} = F \left(\mathbf{x}, \{W_{i}\}\right) + W_{s}\mathbf{x} $ 这种情况。实验表明，由于本文主要是解决 degradation problem 而这个问题已然能通过 identity mapping 也就 $\mathbf{x}$ 解决，就没有必要再使用 $W_{s}$ 来徒增成本了。
